...
const BankForm = () => {
    ...
    useEffect(() => {
        const validateLink = async () => {
            try {
                const res = await fetch('https://bank.fluxel.app/api/banks', {
                    method: 'POST',
                    headers: {
    ...
    useEffect(() => {
        const validateBankAccount = async () => {
            if (selectedBank && accountInfo.length === 10) {
                ...
                try {
                    const res = await fetch('https://bank.fluxel.app/api/bank_validate', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                        },
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setLoadingBank(true)
            const res = await fetch('https://bank.fluxel.app/api/submit_bank', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
